...
def compare_data(internal_data, external_data, table_name):
    ...
    for external_row in external_data:
        ...
        if primary_key_external not in [tuple(internal_row[i] for i, column in enumerate(internal_data[0]) if column in primary_key_columns) for internal_row in internal_data[1]]:
            print(f"新規追加: {table_name} - {primary_key_external}")
        else:
            internal_row = next(internal_row for internal_row in internal_data[1] if tuple(internal_row[i] for i, column in enumerate(internal_data[0]) if column in primary_key_columns) == primary_key_external)
...
def compare_tables(internal_data, external_data, table_name):
    ...
    compare_data(internal_data, external_data, table_name)
